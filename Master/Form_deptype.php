<html>
<head>
<title>Entry Form for deptype</title>
</head>
...
if ( a!="")
{
myform.action="Form_deptype.php?tag=2&ptype=0";
myform.submit();
}
}
...
var c=myform.Sl.value ;
if ( notNull(a) && validateString(a) && a_length<=1 && 1==1 && validateString(b) && b_length<=100 && 1==1)
{
myform.action="Insert_deptype.php";
myform.submit();
}
else
alert('Invalid Data');
...
$mvalue[0]="";//Code
$mvalue[1]="";//Name
// Call $objDeptype->MaxSl() Function Here if required and Load in $mvalue[2]
$mvalue[2]=$objDeptype->MaxSl();
...
$pkarray[0]=0;
$objDeptype->setCode($pkarray[0]);
if ($objDeptype->EditRecord()) //i.e Data Available
{ 
if ($ptype==0) //Post Back on Edit Button Click
{
$mvalue[0]=$objDeptype->getCode();
$mvalue[1]=$objDeptype->getName();
$mvalue[2]=$objDeptype->getSl();
$mvalue[3]=0;//last Select Box for Editing
} //ptype=0
$_SESSION['update']=1;
} 
...
<form name=myform action=insert_deptype.php  method=POST >
<tr><td colspan=2 align=Center bgcolor=#ccffcc><font face=arial size=3>Entry/Edit Form for Department Category<br></font><font face=arial color=red size=2><?php echo  $_SESSION['msg'] ?></font></td></tr>
<?php $i=0; ?>
<tr>
...
<?php $i++; //Now i=1?>
<tr>
<td align=right bgcolor=#FFFFCC><font color=black size=2 face=arial>Name</font></td><td align=left bgcolor=#FFFFCC>
<input type=text size=50 name="Name" id="Name" value="<?php echo $mvalue[$i]; ?>" style="font-family: Arial;background-color:white;color:black; font-size: 12px" maxlength=100 onfocus="ChangeColor('Name',1)"  onblur="ChangeColor('Name',2)">
</td>
</tr>
<?php $i++; //Now i=2?>
<tr>
<td align=right bgcolor=#FFFFCC><font color=black size=2 face=arial>Slno</font></td><td align=left bgcolor=#FFFFCC>
<input type=text size=8 name="Sl" id="Sl" value="<?php echo $mvalue[$i]; ?>" onfocus="ChangeColor('Sl',1)"  onblur="ChangeColor('Sl',2)">
</td>
</tr>
<?php $i++; //Now i=3?>
