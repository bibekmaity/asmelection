...
$i=0;
$tRow=array();
$sql="select Code as Expr,Code from t where ".$this->condString;
$result=mysql_query($sql);
while ($row=mysql_fetch_array($result))
{
$tRow[$i]['Code as Expr']=$row['Code as Expr'];//Primary Key-1
$tRow[$i]['Code']=$row['Code'];//Posible Unique Field
$i++;
}
return($tRow);
}
...
{
$tRows=array();
$sql="select Code from t where ".$this->condString." LIMIT ".$totrec;
$i=0;
$result=mysql_query($sql);
while ($row=mysql_fetch_array($result))
{
