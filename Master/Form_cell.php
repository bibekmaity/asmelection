<html>
<head>
<title>Entry Form for cell</title>
</head>
...
if ( isNaN(a)==false && a!="")
{
myform.action="Form_cell.php?tag=2&ptype=0";
myform.submit();
}
}
...
$mvalue[2]=0;
$mvalue[0]=$objCell->MaxCode();
}
else
{
$mvalue[0]=$objCell->MaxCode();//Code
$mvalue[1]="";//Name
$mvalue[2]=0;
}//end isset mvalue
...
$_SESSION['update']=0;
$_SESSION['msg']="";
// Call $objCell->MaxCode() Function Here if required and Load in $mvalue[0]
$mvalue[0]=$objCell->MaxCode();//Code
$mvalue[1]="";//Name
...
$pkarray[0]=0;
$objCell->setCode($pkarray[0]);
if ($objCell->EditRecord()) //i.e Data Available
{ 
if ($ptype==0) //Post Back on Edit Button Click
{
$mvalue[0]=$objCell->getCode();
$mvalue[1]=$objCell->getName();
$mvalue[2]=0;//last Select Box for Editing
} //ptype=0
$_SESSION['update']=1;
} 
...
<form name=myform action=insert_cell.php  method=POST >
<tr><td colspan=2 align=Center bgcolor=#ccffcc><font face=arial size=3>Entry/Edit Form for Cell<br></font><font face=arial color=red size=2><?php echo  $_SESSION['msg'] ?></font></td></tr>
<?php $i=0; ?>
<tr>
<td align=right bgcolor=#FFFFCC><font color=black size=2 face=arial>Code</font></td><td align=left bgcolor=#FFFFCC>
<input type=text size=8 name="Code" id="Code" value="<?php echo $mvalue[$i]; ?>" onfocus="ChangeColor('Code',1)"  onblur="ChangeColor('Code',2)" onchange=direct1()>
...
<tr><td align=right>
<?php 
$objCell->setCondString(" code>0" ); //Change the condition for where clause accordingly
$row=$objCell->getRow();
?>
