...
$colUpdated=0;
$updateList="";
$sql=" select count(*) from priority";
...
{
$sql=" select count(*) from priority where ".$condition;
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
if (strlen($row[0])>0)
return($row[0]);
...
$i=0;
$tRow=array();
$sql="select code,Detail from priority where ".$this->condString." order by code";
$result=mysql_query($sql);
...
{
$sql="select Code,Detail from priority where Code='".$this->Code."'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
if ($row)
{
...
{
$sql="delete from priority where Code='".$this->Code."'";
$result=mysql_query($sql);
$this->rowCommitted= mysql_affected_rows();
return($result);
$this->returnSql=$sql;
...
{
$sql="insert into priority(Code,Detail) values(";
...
{
$sql="select max(Code) from priority";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
if (strlen($row[0])>0)
return($row[0]+1);
...
{
$tRows=array();
$sql="select Code,Detail from priority where ".$this->condString;
$i=0;
$result=mysql_query($sql);
while ($row=mysql_fetch_array($result))
{
