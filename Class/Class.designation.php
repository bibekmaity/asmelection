...
public function PersonCount($dep,$desig)
{
$sql=" select count(*) from poling where desig='".$desig."' and depcode in(select depcode from department where dep_type='".$dep."')";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
if (strlen($row[0])>0)
return($row[0]);
...
public function DesigWisePidList($dep,$desig)
{
$sql="select Slno from poling where desig='".$desig."' and depcode in(select depcode from department where dep_type='".$dep."')";
$result=mysql_query($sql);
$tmp="(";
